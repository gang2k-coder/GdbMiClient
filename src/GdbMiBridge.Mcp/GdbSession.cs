...
public class GdbSession : IDisposable
{
    ...
    // ═══════════ MCP Tool entry points (thread-safe) ═══════════

    public Task<SessionInfo> CreateAsync(string exe, string? args, string? workDir, bool stopAtEntry)
        => PostAsync<SessionInfo>(new SessionOperation.Create(exe, args, workDir, stopAtEntry, new()));

    public Task<SessionInfo> AttachAsync(int pid)
        => PostAsync<SessionInfo>(new SessionOperation.Attach(pid, new()));

    public Task<SessionInfo> LoadDumpAsync(string path)
        => PostAsync<SessionInfo>(new SessionOperation.LoadDump(path, new()));

    public Task<string> DetachAsync()
        => PostAsync<string>(new SessionOperation.Detach(new()));

    public Task<string> TerminateAsync()
        => PostAsync<string>(new SessionOperation.Terminate(new()));

    public Task<SessionStatus> StatusAsync()
        => PostAsync<SessionStatus>(new SessionOperation.Status(new()));

    public Task<string> GoAsync(int timeoutMs)
        => PostAsync<string>(new SessionOperation.Go(timeoutMs, new()));

    public Task<string> StepIntoAsync()
        => PostAsync<string>(new SessionOperation.StepInto(new()));

    public Task<string> StepOverAsync()
        => PostAsync<string>(new SessionOperation.StepOver(new()));

    public Task<string> StepOutAsync()
        => PostAsync<string>(new SessionOperation.StepOut(new()));

    public Task<string> GoToAsync(string location)
        => PostAsync<string>(new SessionOperation.GoTo(location, new()));

    public async Task<BreakpointConfig> SetBreakpointAsync(string loc, bool capture, string action, string? cond)
    {
        var tcs = new TaskCompletionSource<BreakpointConfig>();
        await _channel.Writer.WriteAsync(new SessionOperation.SetBreakpoint(loc, capture, action, cond, tcs));
        return await tcs.Task;
    }

    public Task<bool> RemoveBreakpointAsync(string id)
        => PostAsync<bool>(new SessionOperation.RemoveBreakpoint(id, new()));

    public Task<bool> EnableBreakpointAsync(string id, bool enabled)
        => PostAsync<bool>(new SessionOperation.EnableBreakpoint(id, enabled, new()));

    public Task<Dictionary<string, string>> GetRegistersAsync()
        => PostAsync<Dictionary<string, string>>(new SessionOperation.GetRegisters(new()));

    public Task<MemoryData> ReadMemoryAsync(string address, int size)
        => PostAsync<MemoryData>(new SessionOperation.ReadMemory(address, size, new()));

    public Task<List<GdbMi.FrameInfo>> GetCallStackAsync(int maxFrames)
        => PostAsync<List<GdbMi.FrameInfo>>(new SessionOperation.GetCallStack(maxFrames, new()));

    public Task<List<ThreadInfo>> ListThreadsAsync()
        => PostAsync<List<ThreadInfo>>(new SessionOperation.ListThreads(new()));

    public Task<List<VariableInfo>> GetLocalVariablesAsync(int frameIndex)
        => PostAsync<List<VariableInfo>>(new SessionOperation.GetLocalVariables(frameIndex, new()));

    public Task<ProgramCounterInfo> GetProgramCounterAsync()
        => PostAsync<ProgramCounterInfo>(new SessionOperation.GetProgramCounter(new()));

    public Task<CaptureResult> CaptureStateAsync()
        => PostAsync<CaptureResult>(new SessionOperation.CaptureState(new()));

    public Task<SymbolInfo> ResolveSymbolAsync(string name)
        => PostAsync<SymbolInfo>(new SessionOperation.ResolveSymbol(name, new()));

    public Task<SymbolInfo> AddressToSymbolAsync(string address)
        => PostAsync<SymbolInfo>(new SessionOperation.AddressToSymbol(address, new()));

    public Task<List<SymbolInfo>> FindSymbolsAsync(string pattern)
        => PostAsync<List<SymbolInfo>>(new SessionOperation.FindSymbols(pattern, new()));

    public Task<List<DisassemblyLine>> DisassembleAsync(string address, int count)
        => PostAsync<List<DisassemblyLine>>(new SessionOperation.Disassemble(address, count, new()));

    public Task<List<ModuleInfo>> ListModulesAsync()
        => PostAsync<List<ModuleInfo>>(new SessionOperation.ListModules(new()));

    public Task<string> RawGdbAsync(string command)
        => PostAsync<string>(new SessionOperation.RawGdb(command, new()));
    ...
    private async Task ConsumerLoopAsync(CancellationToken ct)
    {
        try
        {
            ...
            {
                var op = await _channel.Reader.ReadAsync(ct);
    ...
    private async Task ProcessOperationAsync(SessionOperation op)
    {
        switch (op)
        {
            ...
            case SessionOperation.AddressToSymbol as2: await HandleAddressToSymbol(as2); break;
    ...
    private async Task HandleCreate(SessionOperation.Create c)
    {
        ...
        c.Completion.TrySetResult(new SessionInfo("create", _transport.DebuggerPid, null));
    }

    private async Task HandleDetach(SessionOperation.Detach d)
    {
        await _cmd!.TargetDetach();
        CleanupSession();
        d.Completion.TrySetResult("detached");
    }

    private async Task HandleTerminate(SessionOperation.Terminate t)
    {
        await _cmd!.Terminate();
        CleanupSession();
        t.Completion.TrySetResult("terminated");
    }
    ...
    private async Task HandleGo(SessionOperation.Go g)
    {
        ...
        if (g.TimeoutMs > 0)
        {
            ...
            _ = Task.Run(async () =>
            {
                try { await Task.Delay(g.TimeoutMs, _goTimeoutCts.Token);
                    await _client!.ExecuteAsync(new GdbMi.MICommand("-exec-interrupt", "")); }
                catch { }
            });
        }
    ...
    private async Task HandleSetBreakpoint(SessionOperation.SetBreakpoint sb)
    {
        ...
        _bpManager.Register(number, new BreakpointConfig(number, sb.Location, sb.Capture, sb.Action, sb.Condition, true));
        sb.Completion.TrySetResult(new BreakpointConfig(number, sb.Location, sb.Capture, sb.Action, sb.Condition, true));
    }
    ...
    private async Task HandleReadMemory(SessionOperation.ReadMemory rm)
    {
        try
        {
            var result = await _client!.ConsoleCmdAsync(
                $"x/{rm.Size}xb {rm.Address}", allowWhileRunning: false);
            rm.Completion.TrySetResult(new MemoryData(rm.Address, rm.Size, result.Trim(), Array.Empty<byte>(), ""));
        }
        catch { rm.Completion.TrySetResult(new MemoryData(rm.Address, 0, "", Array.Empty<byte>(), "")); }
    }

    private async Task HandleGetCallStack(SessionOperation.GetCallStack gcs)
    {
        var frames = await _cmd!.StackListFrames(_currentThread, 0, (uint)gcs.MaxFrames);
        gcs.Completion.TrySetResult(FramesToList(frames));
    }
    ...
    private async Task HandleListThreads(SessionOperation.ListThreads lt)
    {
        ...
        var threadList = result.Find<GdbMi.ResultListValue>("threads");
        foreach (GdbMi.TupleValue t in threadList.FindAll<GdbMi.TupleValue>("thread"))
        {
            threads.Add(new ThreadInfo(t.FindInt("id"),
                t.FindString("id") == result.TryFindString("current-thread-id")));
        }
    ...
    private async Task HandleGetProgramCounter(SessionOperation.GetProgramCounter gpc)
    {
        try
        {
            ...
            gpc.Completion.TrySetResult(new ProgramCounterInfo(
                f.TryFindString("addr") ?? "", f.TryFindString("func") ?? "", ""));
        }
        catch { gpc.Completion.TrySetResult(new ProgramCounterInfo("?", "?", "")); }
    }
    ...
    {
        var result = await _client!.ConsoleCmdAsync($"info address {rs.Name}", allowWhileRunning: false);
        rs.Completion.TrySetResult(new SymbolInfo(rs.Name, ExtractAddress(result) ?? "unknown"));
    }

    private async Task HandleAddressToSymbol(SessionOperation.AddressToSymbol a2s)
    {
        var result = await _client!.ConsoleCmdAsync($"info symbol {a2s.Address}", allowWhileRunning: false);
        a2s.Completion.TrySetResult(new SymbolInfo(result.Trim(), a2s.Address));
    }

    private async Task HandleFindSymbols(SessionOperation.FindSymbols fs)
    {
        var result = await _client!.ConsoleCmdAsync($"info functions {fs.Pattern}", allowWhileRunning: false);
        fs.Completion.TrySetResult(ParseSymbolLines(result));
    }

    private async Task HandleDisassemble(SessionOperation.Disassemble d)
    {
        var result = await _client!.ConsoleCmdAsync($"disassemble {d.Address},+{d.Count * 4}", allowWhileRunning: false);
        d.Completion.TrySetResult(ParseDisassembly(result));
    }

    private async Task HandleListModules(SessionOperation.ListModules lm)
    {
        var result = await _client!.ConsoleCmdAsync("info sharedlibrary", allowWhileRunning: false);
        // Simple parsing: return raw output as single module entry
        lm.Completion.TrySetResult(new List<ModuleInfo> { new("shared libraries", result.Trim(), 0) });
    }

    private async Task HandleRawGdb(SessionOperation.RawGdb rg)
    {
        var result = await _client!.ConsoleCmdAsync(rg.Command, allowWhileRunning: true);
        ...
        while (_transport is not null && !_transport.IsClosed)
        {
            ...
            if (line.StartsWith('*'))
            {
                ...
                if (content.StartsWith("stopped"))
                {
                    ...
                    if (_waitingForStop is null) continue; // go-action: keep reading
                    return; // break-action: done
                }
            }
            if (_client is not null) _client.ProcessLine(line);
        }
    }
    ...
    private async Task<CaptureResult> CaptureAsync(string bpNumber, string location)
    {
        ...
        try { var d = new Dictionary<string, string>(); foreach (var r in await _cmd!.DataListRegisterValues(_currentThread)) d[r.TryFindString("number") ?? "?"] = r.TryFindString("value") ?? "?"; registers = d; }
        catch { registers = new(); }

        ProgramCounterInfo pc;
        try { var f = (await _cmd!.StackInfoFrame()).Find<GdbMi.TupleValue>("frame"); pc = new(f.TryFindString("addr") ?? "", f.TryFindString("func") ?? "", ""); }
        catch { pc = new("", "", ""); }

        List<GdbMi.FrameInfo> stack;
        try { stack = FramesToList(await _cmd!.StackListFrames(_currentThread)); }
        catch { stack = new(); }

        List<VariableInfo> locals;
        try { locals = ParseVariables(await _cmd!.StackListLocals(1, _currentThread, 0)); }
        catch { locals = new(); }

        return new CaptureResult(bpNumber, location, registers, pc, stack, null, locals, DateTimeOffset.UtcNow);
    }

    private static List<GdbMi.FrameInfo> FramesToList(GdbMi.TupleValue[] frames)
        => frames.Select(f => new GdbMi.FrameInfo(
            f.TryFindString("level") ?? "", f.TryFindString("addr"), f.TryFindString("func"),
            f.TryFindString("file"), f.TryFindUint("line") is { } l ? (int?)l : null)).ToList();

    private static List<VariableInfo> ParseVariables(GdbMi.ResultValue locals)
    {
        ...
        if (locals is GdbMi.ValueListValue vlist)
            foreach (GdbMi.TupleValue t in vlist.AsArray<GdbMi.TupleValue>())
                result.Add(new VariableInfo(t.TryFindString("name") ?? "?",
                    t.TryFindString("type") ?? "?", t.TryFindString("value") ?? "?"));
        return result;
    }

    private static string? ExtractAddress(string output)
    {
        var idx = output.IndexOf("0x", StringComparison.Ordinal);
        if (idx < 0) return null;
        var end = output.IndexOf(' ', idx);
        return end < 0 ? output[idx..] : output[idx..end];
    }

    private static List<SymbolInfo> ParseSymbolLines(string output)
    {
        var list = new List<SymbolInfo>();
        using var sr = new StringReader(output);
        while (sr.ReadLine() is string line)
        {
            var idx = line.IndexOf("0x", StringComparison.Ordinal);
            if (idx > 0) list.Add(new SymbolInfo(line[..idx].Trim(), line[idx..].Trim()));
        }
        return list;
    }
    ...
    private static List<DisassemblyLine> ParseDisassembly(string output)
    {
        ...
        {
            line = line.Trim();
            if (string.IsNullOrEmpty(line) || line.StartsWith("Dump") || line.StartsWith("End")) continue;
            var sp = line.IndexOf(' ');
            if (sp < 0) continue;
            var addr = line[..sp].TrimEnd(':');
            var rest = line[(sp + 1)..];
            var tab = rest.IndexOf('\t');
            lines.Add(new DisassemblyLine(addr, tab > 0 ? rest[..tab].Trim() : "",
                tab > 0 ? rest[(tab + 1)..].Trim() : rest.Trim()));
        }
        return lines;
    }
    ...
    private void CleanupSession()
    {
        ...
        _captures.Clear();
        _transport?.Close();
        _client = null; _cmd = null; _transport = null;
    }
}
