...
public class E2ETests
{
    ...
    [Fact]
    public async Task LoadDump_InspectCore()
    {
        using var loggerFactory = LoggerFactory.Create(b => b.AddConsole().SetMinimumLevel(LogLevel.Debug));
        var logger = loggerFactory.CreateLogger<GdbSession>();
        using var session = new GdbSession(logger);

        // Load core dump (crash_test.core was generated by crash_test program)
        var info = await session.LoadDumpAsync("/tmp/crash_test.core");
        _output.WriteLine($"LoadDump: type={info.Type}, pid={info.ProcessId}");

        // After loading core, we're "stopped" at the crash point
        // Check the call stack — should show crash_me → main
        var stack = await session.GetCallStackAsync(20);
        _output.WriteLine($"Call stack: {stack.Count} frames");
        foreach (var f in stack.Take(5))
            _output.WriteLine($"  {f.Level}: {f.FunctionName} @ {f.Address}");

        Assert.NotEmpty(stack);
        Assert.Contains(stack, f => f.FunctionName != null && f.FunctionName.Contains("crash_me"));
        Assert.Contains(stack, f => f.FunctionName != null && f.FunctionName.Contains("main"));

        // Check registers — should have valid values
        var regs = await session.GetRegistersAsync();
        _output.WriteLine($"Registers: {regs.Count}");
        Assert.NotEmpty(regs);

        // Check program counter — should be inside crash_me at null deref
        var pc = await session.GetProgramCounterAsync();
        _output.WriteLine($"PC: {pc.Symbol} @ {pc.Address}");
        Assert.Contains("crash_me", pc.Symbol);

        // Check local variable 'n' in crash_me — should be 1
        var locals = await session.GetLocalVariablesAsync(frameIndex: 0);
